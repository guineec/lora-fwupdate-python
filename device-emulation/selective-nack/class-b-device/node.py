...
class SRNackClassBDevice:
    def __init__(self, url, spreading_factor=12, nrx_windows=5):
        ...
        self.uplinks = 0
        # self.logger = Logger(output="experiments/1k/sf7/1.json")
        self.logger = Logger(5, output="stdout")
    ...
    def uplink(self):
        ...
        for i in range(0, len(hex_bytes), 50):
            # time.sleep(random.randint(0, 2))  # Random transmission delay simulated
            if i + 50 >= len(hex_bytes) - 1:
                ...
                if number < self.drop_chance:
                    ...
                    rcvd_indices.append(int(byte[2]))
                    self.packets.append(byte[2])
        self.logger.uplink_rcvd(rcvd_indices)
        print(rcvd_indices)
        self.nack_queue = indices
...
dev = SRNackClassBDevice("http://localhost:5001", spreading_factor=7, nrx_windows=5)
dev.run()
